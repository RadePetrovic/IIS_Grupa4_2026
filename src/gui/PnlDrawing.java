...
public class PnlDrawing extends JPanel {
	...
	protected void thisMouseClicked(MouseEvent me) {
		...
		if (selectedShape != null) {
			...
			while (iterator.hasNext()) {
				Shape shape = iterator.next();
				if (shape.contains(click.getX(), click.getY()) != true || me.getClickCount() == 2) {
					shape.setSelected(false);
				}
			}
		}
		...
		if (frame.getTglbtnPoint().isSelected()) {
			...
			if (dialog.isOk()) {
				//  kreirana tačka se uzima iz dijaloga koja u sebi sadrži izabranu boju
			...
			if (startPoint == null) {
				...
				dialog.setVisible(true);
				if (dialog.isOk()) {
				    newShape = dialog.getLine(); // Uzima liniju sa postavljenom bojom iz dijaloga
				}
				startPoint = null; // Resetuje startnu tačku nakon kreiranja linije
			}
